...
> **起手批量并发首读（proposal-012 §3② · 一条消息并发）**：进入阶段9 第一动作 = 把以下独立首读项在**一条消息内并发**读取——HITL-4 确认的部署参数表 + `spec.md`（可自动化验收项）+ `ci_result/ci_result.md`（阶段8 结论）+ `HARNESS_CONFIG.yaml`（启动命令/探活端点）。**部署参数仍禁止推测**（缺参走 HITL-4）。
