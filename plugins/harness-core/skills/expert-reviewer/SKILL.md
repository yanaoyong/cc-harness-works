...
> **阶段2 计划评审 · 审并发组划分（评审 tasks.md 声明了并发组时 · 承接方向④阶段3 照组机械并发）**：tasks.md 若按并发组拆分任务（每任务标注所属并发组 + 该组落点白名单 + 依赖序），阶段2 计划评审须**顺带核对并发组划分**，逐项过——(a) **组内各任务文件落点确不相交**（同一并发组内任务的落点白名单两两无交集，否则阶段3 一条消息内并发多 generator 会写冲突）；(b) **跨组依赖已标依赖序**（有先后依赖 / 落点相交的任务须按 `request-analysis` §4.1.2 唯一裁决处理——**合并为同一任务** 或 **拆到不同并发组显式标明依赖序**，不得被误划入同一并发组并发执行）；(c) **每组落点白名单齐全**（每任务附所属并发组编号 + 该组落点白名单，无遗漏）。任一项不满足 = **MUST FIX**（阶段3 照组机械并发依赖此划分正确，划分错则并发写冲突或依赖倒挂）。并发组字段承载形态与阶段3 照组并发编排见 `_TEMPLATE/request_analysis/tasks.md` 与 `application-owner.md` 模块四——**指针引用、不复制定义**（守单源）。
...
### 4.3 复评（v2+）默认范围与实例续接（承接方向③ · reviewer 热续 + 复评差异化）

> 首评与复评的 reviewer **实例策略**与**评审范围**不同，须严格区分——首评保独立冷启动、复评走热续差异化，以省掉复评的冷启动重读全卡。

- **首评（v1）· 全新冷启动实例（保独立性）**：首次评审**必须由全新冷启动的 reviewer 实例**执行——不得续接任何在制品/上游上下文，以保独立视角、防被 generator 的编码上下文污染（评审独立性铁律）。首评范围 = **全量**（全卡对照 Rules 与需求逐项检查，§4 步骤照常）。
- **复评（v2+）· 热续 + 差异化范围**：v2+ 复评默认走**热续**（续接同一 v1 实例、不冷启动重读全卡），且**默认范围写死收窄为**——**闭合项（v1 MUST FIX 逐条核对）+ 本轮 diff**。即：只做两件事——(1) 逐条核对 v1 提出的每条 MUST FIX 是否已闭合；(2) 审本轮 diff（`git diff` 自 v1 落盘以来的改动）引入的新增内容有无新缺陷。**不再全量重读未改动部分**（未改动部分已在 v1 审过）。
- **续接编排纪律（权威落点指针 · 不重定义）**：热续的 Owner 侧编排——经 `SendMessage` 续同一 v1 实例、「续不上」信号判定与退化为冷启动 v2、R4「reviewer·<档位>·热续」后缀留痕、回合预算沿用 ≤15——权威定义见 `application-owner.md` 模块四；本 SKILL 只放指针、**不重定义**（守单一事实来源 · 防双源漂移）。Owner 经 `SendMessage` 的热续续话**即构成** `.claude/agents/reviewer.md` 严格约束第 4 条「Owner 新一次调用显式启动 v2」、**不违**第 1 条「ONE pass 自启动」禁令（该禁令针对 reviewer 无 Owner 指令的自迭代 · 语义弥合详见 `application-owner.md` 模块四热续编排纪律 (e)）。
- **对抗性不放松（边界 · 硬约束）**：复评范围收窄**仅指"不重读未改动部分"**——对**闭合项与本轮 diff** 的审视仍**全量对抗**：主动构造失败形态、独立复验、按 §4.1 验证档位采样/升级全量档的强度**一个不减**。热续/差异化范围**不是**放松对抗评审的理由（§4 评审步骤与对抗本质不因此打折）。
