...
**（四）HITL-4 预告 HITL-5（AC-5.5）**：HITL-4 部署环境参数确认呈现时，**预告 HITL-5**（最终交付 / merge 授权）即将到来，供用户预期尾程；**但 merge 授权语仍须逐字输入、不并入选项**（不并入 HITL-4 的 `AskUserQuestion` 选项 · AskUserQuestion 作答不落授权台账 · DF-015），与上方（一）push 授权处理对称。
