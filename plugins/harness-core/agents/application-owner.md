...
**R2 · 预判上调白名单**：首次部署、复杂 CI 诊断、涉不可逆操作 / 密钥邻近的推送 → Owner 直接用 sonnet/opus，委派记录留一句理由。
...
**R4 · 留痕观测（硬格式 · P1b 升格 · 来源 `proposal-011` §6）**：`summary.md` **阶段记录行必须带「执行方·档位」标注**——由原「记录该阶段实际使用的模型档」升格为硬格式（proposal-011 五卡实测 R4 留痕基本未执行、审计只能翻 transcript，故硬格式化使执行率可 grep 审计）。**格式样例**：`generator·sonnet` / `reviewer·opus` / `reviewer·sonnet(pilot)` / `owner·inline·<一句理由>`（阶段 7/8/9 Owner inline 亲自执行时的理由即以 `owner·inline·<理由>` 承载，与上方 P0 纪律衔接）。**判据：漏标 = R4 不合规**（summary 阶段记录行 grep 可证执行率，对照 p010「分档纸面化一个月」病灶）。**适用面**：本硬格式**仅 10 阶段卡阶段记录行先行**（OQ-4 决议——proposal-011 审计对象即 10 阶段卡）；元流程 M 阶段记录行留后续卡，暂不受本硬格式约束。

### 阶段6 单测评审 sonnet 试点（P1a · 仅阶段6 · 来源 `proposal-011` §5）
...
1. **双跑对照**：前 **3–5 卡**对阶段 6 单测评审做 **sonnet ∥ opus 双跑**——两评审各评一轮、落**不同文件**（避免相互污染），Owner 比两份结论**差集**；确认 sonnet 无关键漏抓后方可转常态单跑 sonnet。双跑若均含破坏性取证（kill-mutant），**各评审须在各自隔离副本上变异，禁止直接变异工作树共享文件**（试点首跑卡实测竞态：两评审并发变异同一未跟踪被测文件互踩备份，病例见 `chore-cost-discipline-v2-20260712` 阶段6 双份评审报告）。
2. **R1 失败升档梯子照用**：试点期 sonnet 评审若命中 R1 失败信号（门禁未过且根因疑似模型执行质量 / 回报含损坏残片）→ 同任务升档 `sonnet→opus` 重评（对齐下方 R1，不做同档二次重试）。
3. **R4 留痕标 `reviewer·sonnet(pilot)`**：试点期阶段 6 记录行按 R4 硬格式标 `reviewer·sonnet(pilot)`，与常态 `reviewer·opus` 明确区分，供审计辨识试点样本。
4. **退出条件**：双跑期 sonnet **漏抓** ≥1 个 **MUST FIX 级**问题（opus 抓到而 sonnet 未抓的差集非空）→ **试点终止、阶段 6 回 opus**。

**身后兜底**：即便 sonnet 漏抓，阶段 8 CI 冻结判定式（`exit==0 && total>0 && passed==total` · DF-009）与下方 R3 安全底座两道**模型无关门禁**仍在，试点漏抓不产生假绿穿透（proposal-011 §5 兜底论证）。
