...
### 对抗式实跑纪律（评审必猎杀失败形态 · 不靠全绿测试放行）

> 教训背景：某卡编码 v1 引入了比原 bug 更坏的回归（正则身份错认），当时全套单测全绿；逮到它的不是测试，而是评审 prompt 里「主动构造新正则可能漏提/错提号的真实命令形态并逐条实跑」这句对抗指令。同族再证：某 shell 脚本把基线文件里的字符串内插进 `bash -c` 造成命令注入，评审用注入 payload 实跑触达（canary 被创建、脚本仍 exit 0）才坐实——多轮返工无一轮靠测试全绿发现。

- **必须主动构造失败形态并实跑**：评审代码/脚本时**禁止只做静态阅读、或只复述编码 agent 的自测结论**；必须在评审中**主动构造可能失败的输入形态（欠匹配 / 过匹配 / 误伤 / 边界 / 恶意注入）+ 实跑验证 + 贴原样输出**，点名"要猎杀什么"，并想清单之外的形态。
- **必须独立复现 · 禁止仅凭报告下结论**：编码 agent 报告"自测全过"**不作评审判据**；必须**独立复现**其自测结论，明确不许仅凭报告自述放行。
- **适用性**：本纪律对一切代码/脚本评审（阶段4）通用——尤以正则、字符串拼接、shell 脚本、外部可控输入解析为重点猎杀区；测试全绿 ≠ 无缺陷，评审的对抗性实跑探针是最后一道真防线。
<!-- HARNESS:LESSON:END -->
...
6. **回合预算 ≤15（预算固化 · 对偶提醒）**：Owner 委派 reviewer 时应在 prompt 写明 **回合预算 ≤15**（RM-029 实测曾漏写 reviewer 预算，故由纪律依赖升为模板固化）；reviewer 侧以之为**自觉锚**——超预算是**气味信号非铡刀**，**不机械中止**，**证据完整性 > 简洁**（任何 MUST FIX 或必需证据不得为压缩回合而省略）。数字沿用现值不改。来源：`proposal-011`（P2b · RM-029 漏写病因）。
